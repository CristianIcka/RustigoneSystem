...
class VentasWindow:
    ...
    def solicitar_monto_efectivo(self):
        """Solicitar monto pagado en efectivo con cálculo automático de vuelto y opción para pagos justos"""
        # Crear ventana para ingresar monto
        self.monto_window = ctk.CTkToplevel(self.window)
        self.monto_window.title("Pago en Efectivo")
        from ui.responsive import set_window_size_and_center
        set_window_size_and_center(self.monto_window, width_percent=0.4, min_width=450, min_height=500, parent=self.window)
        ...
        # Entrada de monto con formato automático
        ...
        self.monto_entry = ctk.CTkEntry(
            input_frame,
            height=50,
            placeholder_text="Ingrese monto recibido (ej: 10000)",
        ...
        # Enlazar el evento de teclado para cálculo automático
        self.monto_entry.bind("<KeyRelease>", self.calcular_vuelto_automatico)
        self.monto_entry.bind("<Return>", lambda e: self.confirmar_pago_efectivo())
        self.monto_entry.focus()
        
        # Botón para pagar con lo justo (sin vuelto)
        pago_justo_frame = ctk.CTkFrame(main_frame, fg_color="transparent")
        pago_justo_frame.pack(fill="x", pady=(5, 15))
        
        self.pago_justo_btn = ctk.CTkButton(
            pago_justo_frame,
            text="💰 PAGAR CON LO JUSTO (SIN VUELTO)",
            command=self.pagar_con_lo_justo,
            height=40,
            fg_color="#D69E2E",
            hover_color="#B7791F",
            font=ctk.CTkFont(weight="bold", size=13)
        )
        self.pago_justo_btn.pack(fill="x")
        
        # Marco para mostrar el vuelto
        vuelto_frame = ctk.CTkFrame(main_frame, fg_color="#EDF2F7", corner_radius=10)
        vuelto_frame.pack(fill="x", pady=(0, 15))
        
        # Frame interno para organizar mejor
        vuelto_content = ctk.CTkFrame(vuelto_frame, fg_color="transparent")
        vuelto_content.pack(padx=20, pady=15, fill="both", expand=True)
        
        # Título del vuelto
        vuelto_title_frame = ctk.CTkFrame(vuelto_content, fg_color="transparent")
        vuelto_title_frame.pack(fill="x", pady=(0, 10))
        ...
        # Valor del vuelto (ocupa el espacio restante)
        ...
        self.vuelto_label.pack(side="right")
        
        # Indicador de estado
        self.estado_label = ctk.CTkLabel(
            vuelto_content,
            text="Ingrese el monto recibido o use 'Pagar con lo justo'",
            font=ctk.CTkFont(size=12),
            text_color="#718096",
            wraplength=400
        )
        self.estado_label.pack(fill="x", pady=(5, 0))
    ...
    def pagar_con_lo_justo(self):
        """Configurar el pago con el monto exacto (sin vuelto)"""
        # Establecer el monto recibido igual al total
        self.monto_recibido_var.set(f"{self.total_venta:,.0f}")
        
        # Calcular y mostrar el vuelto (que será 0)
        self.calcular_vuelto_automatico()
        
        # Enfocar en el campo de entrada
        self.monto_entry.focus()

    def calcular_vuelto_automatico(self, event=None):
        """Calcular automáticamente el vuelto mientras se escribe"""
        try:
            # Obtener el texto limpio
            monto_texto = self.monto_recibido_var.get().strip()
            
            if not monto_texto:
                self.vuelto_label.configure(text="$0", text_color="#718096")
            ...
            monto_texto = re.sub(r'[^\d,.]', '', monto_texto)
            
            # Reemplazar comas por puntos si hay decimales
            if ',' in monto_texto and '.' in monto_texto:
                # Si hay ambos, eliminar comas
                monto_texto = monto_texto.replace(',', '')
            elif ',' in monto_texto:
                # Si solo hay comas, reemplazar por punto
                monto_texto = monto_texto.replace(',', '.')
            ...
            try:
                ...
                self.vuelto_label.configure(text="$0", text_color="#E53E3E")
                self.estado_label.configure(
            ...
            # Formatear el monto en la entrada
            formatted_monto = f"{monto_recibido:,.0f}"
            if self.monto_recibido_var.get() != formatted_monto:
                self.monto_recibido_var.set(formatted_monto)
                # Mover el cursor al final
                self.monto_entry.icursor(len(formatted_monto))
            
            # Calcular vuelto
            vuelto = monto_recibido - self.total_venta
            
            # Actualizar etiquetas según el resultado
            if monto_recibido <= 0:
                self.vuelto_label.configure(text="$0", text_color="#E53E3E")
                self.estado_label.configure(
                ...
                self.estado_label.configure(
                    text="Vuelto listo para entregar",
                    text_color="#2B6CB0"
                )
                self.confirmar_efectivo_btn.configure(state="normal")
                
        except Exception as e:
            print(f"Error calculando vuelto: {e}")
            self.vuelto_label.configure(text="$0", text_color="#718096")
            self.estado_label.configure(
...
"""
    ...
    def mostrar_resumen_venta(self, medio_pago, monto_recibido=0, vuelto=0):
        """Mostrar resumen completo de la venta"""
